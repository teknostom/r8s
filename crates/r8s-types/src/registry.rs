...
impl ResourceRegistry {
    ...
    pub fn default_mvp() -> Self {
        ...
        let types: &[(&str, &str, &str, &str, bool, &str, &[&str])] = &[
            ...
            // Conformance's AggregatedDiscovery test enumerates a fixed list
            // of expected GVRs. Registering the remaining ones as inert
            // (stored opaquely, no controller) is enough — the tests only
            // check that the resources appear in /api(s) discovery.
            (
