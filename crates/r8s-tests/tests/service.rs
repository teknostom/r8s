...
async fn endpoints_from_service_selector() {
    ...
    cluster.create(&deploy_gvr, "default", "ep-web", &make_deployment("ep-web", 2, "ep-web"));

    let pods_running = wait_for_count(
        &cluster.store, &pod_gvr, Some("default"),
        |v| has_label(v, "app", "ep-web") && has_phase(v, "Running"), 2, TIMEOUT,
    ).await;
    ...
    let ep_found = wait_for(
        &cluster.store, &ep_gvr, Some("default"), "ep-svc",
        |v| v["subsets"][0]["addresses"].as_array().map(|a| a.len() >= 2).unwrap_or(false),
        TIMEOUT,
    ).await;
...
async fn endpoints_update_on_pod_change() {
    ...
    cluster.create(&deploy_gvr, "default", "ep-upd", &make_deployment("ep-upd", 1, "ep-upd"));

    let running = wait_for_count(
        &cluster.store, &pod_gvr, Some("default"),
        |v| has_label(v, "app", "ep-upd") && has_phase(v, "Running"), 1, TIMEOUT,
    ).await;
    ...
    let ep1 = wait_for(
        &cluster.store, &ep_gvr, Some("default"), "ep-upd-svc",
    ...
    let three_running = wait_for_count(
        &cluster.store, &pod_gvr, Some("default"),
        |v| has_label(v, "app", "ep-upd") && has_phase(v, "Running"), 3, TIMEOUT,
    ).await;
...
async fn endpoints_only_running_pods() {
    ...
    cluster.create(&deploy_gvr, "default", "ep-run", &make_deployment("ep-run", 1, "ep-run"));

    let running = wait_for_count(
        &cluster.store, &pod_gvr, Some("default"),
        |v| has_label(v, "app", "ep-run") && has_phase(v, "Running"), 1, TIMEOUT,
    ).await;
    assert!(running, "1 pod should be Running");
    ...
    let ep_ok = wait_for(
        &cluster.store, &ep_gvr, Some("default"), "ep-run-svc",
        |v| v["subsets"][0]["addresses"].as_array().map(|a| a.len() == 1).unwrap_or(false),
        TIMEOUT,
    ).await;
...
async fn endpointslice_created() {
    ...
    cluster.create(&deploy_gvr, "default", "ep-slice", &make_deployment("ep-slice", 1, "ep-slice"));

    let running = wait_for_count(
        &cluster.store, &pod_gvr, Some("default"),
        |v| has_label(v, "app", "ep-slice") && has_phase(v, "Running"), 1, TIMEOUT,
    ).await;
    assert!(running, "1 pod should be Running");
    ...
    let es_found = wait_for(
        &cluster.store, &es_gvr, Some("default"), "ep-slice-svc",
...
async fn kubelet_readiness_probe_gates_endpoints() {
    ...
    let running = wait_for(
        &cluster.store, &pod_gvr, Some("default"), "probe-unready",
    ...
    let ep_created = wait_for(
        &cluster.store, &ep_gvr, Some("default"), "probe-unready-svc",
        |_| true, TIMEOUT,
    ).await;
    ...
    assert_eq!(addr_count, 0, "pod with failing readiness probe should not appear in Endpoints");
