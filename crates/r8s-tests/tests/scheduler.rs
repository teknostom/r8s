...
async fn scheduler_registers_node() {
    ...
    let node = cluster.store
        .get(&r8s_store::backend::ResourceRef {
            gvr: &node_gvr, namespace: None, name: "r8s-node",
        })
        .unwrap();
...
async fn scheduler_sets_pod_scheduled_condition() {
    ...
    cluster.create(&pod_gvr, "default", "sched-test", &make_pod("sched-test", "nginx:latest"));

    // Wait for PodScheduled=True condition
    let scheduled = wait_for(
        &cluster.store, &pod_gvr, Some("default"), "sched-test",
        |v| has_condition(v, "PodScheduled", "True"), TIMEOUT,
    ).await;
...
async fn scheduler_skips_already_scheduled() {
    ...
    assert_eq!(
        val["spec"]["nodeName"].as_str(), Some("other-node"),
