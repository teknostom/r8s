...
async fn daemonset_creates_pod() {
    ...
    cluster.create(&gvr, "default", "node-agent", &make_daemonset("node-agent", "node-agent"));
    ...
    let status_ok = wait_for(
        &cluster.store, &gvr, Some("default"), "node-agent",
        |v| {
...
async fn daemonset_pod_replacement() {
    ...
    cluster.create(&gvr, "default", "ds-replace", &make_daemonset("ds-replace", "ds-replace"));
